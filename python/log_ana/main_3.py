...
def same_sid_click(dict_name):#传入的字典结构：dict_name{int:{"key":"value"}}
	...
	for k in dict_name:
		...
		if dict_name[k]["SESSION_ID"] not in sid:#用in来避免重复，
			sid.append(dict_name[k]["SESSION_ID"])
			for k1 in dict_name_2:
				if dict_name[k]["SESSION_ID"] == dict_name_2[k1]["SESSION_ID"]:
					add = [dict_name_2[k1]["F_TIME"],dict_name_2[k1]["F_URL"]]#append只有一个参数，换成append需做两次，现在的做法，add在每次被重置为最新值，没有积累
					add_1.append(add[0])
					x.append(str(add))#先将列表转成字符串以方便ＨＡＳＨ，同样append只有一个参数
					...
					'''
						...
						打印结果
					'''
			if len(s) > 0 :#过滤没有任何点击的搜索
				print '%s, 来自城市　%s 的用户　%s　共点击了 %s 次搜索结果，分别如下:' % (dict_name[k]["F_DATE"],dict_name[k]["F_CITY"],dict_name[k]["F_MOBILE"],len(s))
				fetch_list = []
				for i in s:#将集合s的每一项转化为一个列表
				...
				for f1 in s_2:
					...
					#print '在北京时间%s，点击了%s'%(fetch_list[0],fetch_list[1])
					#print '在北京时间%s，点击了'% fetch_list[0]
					#print '在北京时间%s，点击了%s'%(dict_name[k1]["F_TIME"],i)
				print "-----------------------------------------------------------------------\n"
