...
//开发者通过检验signature对请求进行校验（下面有校验方式）。若确认此次GET请求来自微信服务器，请求原样返回echostr参数内容，则接入生效，否则接入失败。
	    ...
	    public function responseMsg()//处理并回复用户发送过来的消息，也是用的最多的一个函数，几乎所有的功能都在这里实现
    {
		...
		$postStr = file_get_contents("php://input");//接收微信公众平台发送过来的用户消息，该消息数据结构为XML，不是php默认的识别数据类型，因此这里用了$GLOBALS['HTTP_RAW_POST_DATA']来接收，同时赋值给了$postStr
      	//extract post data
		if (!empty($postStr)){
...
//以上接收用户输入，以下定义服务器返回格式，可将输入处理后通过指定格式返回。
                $textTpl = "<xml>
